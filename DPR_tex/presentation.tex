...
\subsection{Entrepreneuriat et valorisation}

\begin{frame}[t]{Entrepreneuriat --- Vision de valorisation}
\slidepurpose{Introduire simplement comment une idee technique peut devenir un projet entrepreneurial}{Ouverture business / entrepreneuriat}
\scriptsize
\begin{block}{Le point de depart}
\begin{itemize}
\setlength{\itemsep}{0.22em}
\item Les hopitaux et centres d'imagerie doivent traiter beaucoup d'examens en peu de temps.
\item Une radiographie thoracique peut etre difficile a interpreter rapidement.
\item Un outil d'aide peut donc avoir une vraie utilite pratique.
\end{itemize}
\end{block}

\begin{columns}[T,onlytextwidth]
\begin{column}{0.48\textwidth}
\begin{block}{Notre idee}
\begin{itemize}
\setlength{\itemsep}{0.18em}
\item signaler les cas suspects,
\item aider a prioriser les urgences,
...
\item une aide a la lecture,
\item un service utile pour les structures medicales.
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
On ne vend pas un ``medecin automatique'', mais une \textbf{solution d'assistance} qui aide les professionnels a travailler plus vite et plus sereinement.
\end{alertblock}
\end{frame}

\begin{frame}[t]{Entrepreneuriat --- A qui vendre le projet ?}
\slidepurpose{Identifier les clients potentiels avec un vocabulaire simple}{Etude de marche / premiers concepts}
\scriptsize
\begin{columns}[T,onlytextwidth]
...
\begin{itemize}
\setlength{\itemsep}{0.22em}
\item hopitaux publics et prives,
\item cliniques,
\item centres de radiologie,
\item plateformes de teleradiologie.
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.47\textwidth}
\begin{block}{Leur besoin}
\begin{itemize}
\setlength{\itemsep}{0.22em}
\item gagner du temps,
\item mieux organiser les priorites,
\item reduire le risque d'oubli,
\item utiliser un outil simple.
\end{itemize}
\end{block}
\end{column}
\end{columns}

\begin{block}{Comment presenter le projet ?}
Comme une \textbf{aide au triage}, une \textbf{seconde lecture} et un \textbf{outil explicable} pour la radiographie thoracique.
\end{block}

\begin{alertblock}{Positionnement}
Le projet se vend en \textbf{B2B} : a des etablissements de sante, et non directement au grand public.
\end{alertblock}
\end{frame}

\begin{frame}[t]{Entrepreneuriat --- Comment vendre concretement le projet ?}
\slidepurpose{Montrer simplement les etapes commerciales possibles}{Demarche commerciale / entrepreneuriat debutant}
\scriptsize
\begin{block}{Etapes simples}
\begin{enumerate}
\setlength{\itemsep}{0.34em}
\item \textbf{Presenter le probleme} du client
\item \textbf{Montrer une demonstration} du systeme
\item \textbf{Proposer un petit pilote}
\item \textbf{Recueillir les retours}
\item \textbf{Transformer le test en offre}
\end{enumerate}
\end{block}
...
\begin{tikzpicture}[
    x=0.95cm, y=0.95cm,
    box/.style={draw, rounded corners=4pt, align=center, minimum width=2cm, minimum height=0.85cm, fill=blue!10, font=\scriptsize},
    arr/.style={-{Latex[length=2.5mm]}, thick, draw=blue!70!black}
]
    \node[box] (a) at (0,0) {Besoin};
    \node[box] (b) at (2.6,0) {Demo};
    \node[box] (c) at (5.2,0) {Pilote};
    \node[box] (d) at (7.8,0) {Retours};
    \node[box] (e) at (10.4,0) {Offre};
    \draw[arr] (a) -- (b);
    \draw[arr] (b) -- (c);
    \draw[arr] (c) -- (d);
    \draw[arr] (d) -- (e);
...
Pour un premier cours d'entrepreneuriat, il suffit deja de montrer \textbf{qui peut acheter}, \textbf{comment tester} et \textbf{comment vendre}.
\end{alertblock}
\end{frame}

\begin{frame}[t]{Entrepreneuriat --- Budget simple et modalites de vente}
\slidepurpose{Donner une vision accessible du budget et des revenus possibles}{Faisabilite economique / modele simple}
\scriptsize
\begin{columns}[T,onlytextwidth]
...
Hebergement / cloud / GPU & 30\,000 MAD \\ \hline
Developpement / amelioration & 50\,000 MAD \\ \hline
Tests / pilote / documentation & 25\,000 MAD \\ \hline
Communication / deplacement & 20\,000 MAD \\ \hline
\textbf{Total simplifie} & \textbf{125\,000 MAD} \\ \hline
\end{tabular}
\end{adjustbox}
\end{column}
...
\item Pilote : 10\,000 a 20\,000 MAD
\item Abonnement : 3\,000 a 6\,000 MAD / mois
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
Pour notre niveau, l'essentiel est de montrer qu'il existe un \textbf{marche possible}, un \textbf{mode de vente simple} et un \textbf{budget de depart raisonnable}.
\end{alertblock}
\end{frame}
