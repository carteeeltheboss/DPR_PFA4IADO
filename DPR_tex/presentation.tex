...
\begin{itemize}
\setlength{\itemsep}{0.05em} % Tightened item separation
\item \textbf{Complexité visuelle} des pathologies pulmonaires sur CXR.
\item \textbf{Pénurie mondiale} de radiologues (OMS : 1/100\,000 hab. en Afrique).
\item \textbf{Temps d'attente} critiques aux urgences.
\item IA SOTA actuelles = \textbf{boîtes noires} opaques, freinant l'adoption clinique.
\end{itemize}
\end{block}
\end{column}

\hfill % Evenly distributes space between columns

\begin{column}{0.46\textwidth}
\begin{block}{Origine \& Solution}
\begin{itemize}
\setlength{\itemsep}{0.05em} % Tightened item separation
\item Observation terrain en \textbf{milieu hospitalier}.
\item \textbf{MedFusionNet} : avantage concurrentiel unique :
\begin{itemize}
\setlength{\itemsep}{0.02em}
\item[\textbullet] Double lecture \textbf{locale/globale} (CNN + Swin).
\item[\textbullet] \textbf{Explicabilité} Grad-CAM intégrée.
\item[\textbullet] \textbf{Incertitude} (MC-Dropout) pour sécuriser le diagnostic.
\end{itemize}
\end{itemize}
\end{block}
\end{column}
...
\vspace{-0.3em} % Pulls the alert block up slightly to save space
...
\begin{columns}[T,onlytextwidth]
\begin{column}{0.46\textwidth}
\begin{block}{Demande \& Cibles}
\begin{itemize}
\setlength{\itemsep}{0.05em}
\item \textbf{Hôpitaux publics} : optimiser le triage et réduire les erreurs.
\item \textbf{Cliniques privées} : accélérer les flux et se différencier.
\item \textbf{Centres d'urgence} : aide à la décision h24.
\end{itemize}
\end{block}

\vspace{-0.2em}

\begin{block}{Synthèse SWOT}
\begin{itemize}
\setlength{\itemsep}{0.02em}
\item \textbf{F} : Technologie unique, explicable et quantifiée.
\item \textbf{W} : Statut startup, volume de données cliniques initial.
\item \textbf{O} : Digitalisation et réformes de la santé au Maroc.
\item \textbf{M} : Réglementation stricte, acteurs mondiaux.
\end{itemize}
\end{block}
\end{column}
...
\begin{block}{Analyse PESTEL}
\begin{itemize}
\setlength{\itemsep}{0.02em}
\item \textbf{P/E} : Soutien e-santé ; marché mondial $>$45Mds\$.
\item \textbf{S/T} : Adoption médicale ; GPU cloud \& SOTA matures.
\item \textbf{E/L} : Green IT ; Loi 09-08 / RGPD (données patients).
\end{itemize}
\end{block}

\vspace{-0.2em}

\begin{block}{Forces de Porter}
\begin{itemize}
\setlength{\itemsep}{0.02em}
\item \textbf{Entrants} : Barrière élevée (expertise IA + médicale).
\item \textbf{Clients} : Pouvoir fort $\Rightarrow$ Modèle SaaS flexible.
\item \textbf{Substituts} : Rares (pas d'outil explicable local).
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
\end{block}
\vspace{-0.2em}
\begin{block}{Charges variables (CV)}
\begin{itemize}
\setlength{\itemsep}{0.02em}
\item $\approx$ 0,50 MAD / scan d'API Cloud.
\item Pour 10\,000 scans/mois $\Rightarrow$ \textbf{5\,000 MAD}.
\end{itemize}
\end{block}
\end{column}
...
\begin{itemize}
\setlength{\itemsep}{0.15em}
\item \textbf{Marge brute (MB)} :\\ 
$MB = CA - CV = 85k - 5k = \mathbf{80\,000}$ MAD
\item \textbf{Taux de MCV (TMCV)} :\\ 
$TMCV = \frac{MB}{CA} = \frac{80\,000}{85\,000} \approx \mathbf{94,1\%}$
\item \textbf{Seuil de rentabilité (SR)} :\\ 
$SR = \frac{CF}{TMCV} = \frac{45\,000}{0,941} \approx \mathbf{47\,822}$ MAD/mois
\item \textbf{BFR maîtrisé} : Paiement SaaS d'avance $\Rightarrow$ BFR proche de zéro.
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
Avec un CA cible de 85\,000 MAD et un SR à $\sim$48\,000 MAD, le projet atteint son \textbf{équilibre dès le 8\textsuperscript{e} mois} d'activité commerciale.
\end{alertblock}
...
\begin{columns}[T,onlytextwidth]
\begin{column}{0.46\textwidth}
\begin{block}{Forme Juridique (SARL) \& Conformité}
\begin{itemize}
\setlength{\itemsep}{0.05em}
\item \textbf{Choix} : Structure stable, capital social libre.
\item \textbf{Avantages} : Protection du patrimoine des associés et souplesse adaptée à une startup tech au Maroc.
\item \textbf{Conformité} : Respect strict de la \textbf{Loi 09-08} (anonymisation, hébergement sécurisé, consentement).
\end{itemize}
\end{block}
\end{column}
...
\item \textbf{Moyen terme} : Extension multi-pathologies (tumeurs, tuberculose) \& Certification Dispositif Médical (\textbf{CE / FDA}).
\item \textbf{Long terme} : Intégration PACS/RIS (\textbf{DICOM}), expansion en Afrique francophone, levée de fonds.
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
De projet de recherche à \textbf{startup e-health} certifiée : MedFusionNet vise à devenir une référence du diagnostic IA au Maroc et en Afrique.
\end{alertblock}
...
\subsection{Confiance et explicabilité}
\begin{frame}{Grad-CAM : schéma de génération}
